...
function Viewtask() {
  ...
  const columns = [
    ...
    { field: 'quantity', headerName: 'Total Quantity', width: 100 },
    { field: 'date', headerName: 'Date', width: 120, valueGetter: (params) => formatDateString(params.value) },
  ...
  const sortedTasks = alltasks.slice().sort((a, b) => a.jobid - b.jobid);
  ...
  return (
    <div className='task' style={{ background: 'white', minHeight: '100vh', color: 'white' }}>
      ...
      <div className='taskbox' style={{ width: '70%' }}>
        <div className='toggle-view' style={{marginTop:"10px",color:"black",paddingLeft:"50px"}}>
          <input type="checkbox" checked={isGridView} onChange={toggleView} />
          <label>Change View</label>
        </div>
        {!isMobileView && isGridView ? (
          <DataGrid
            rows={alltasks}
            columns={columns}
            autoHeight
              ...
              {sortedTasks.map((task) => (
                ...
                >
                  <div style={{ textAlign: 'center' }}>
                    <div className='bg-primary' style={{ padding: '5px' }}>
                      <h3>Job id: {task.jobid}</h3>
                    </div>
                    <p>Machine Name: {task.machinename}</p>
                    <p>Status: {task.status}</p>
                    <p>Total Quantity: {task.quantity}</p>
                    <p>Date: {formatDateString(task.date)}</p>
                    <p>Time: {task.time}</p>
                  </div>
                  <button
